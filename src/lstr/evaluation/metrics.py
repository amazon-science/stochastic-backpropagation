...
def perframe_average_precision(ground_truth, prediction, class_names, ignore_index, metrics,
                                postprocessing):
    ...
    for idx, class_name in enumerate(class_names):
        if idx not in ignore_index:
            result['per_class_AP'][class_name] = compute_score(
                (ground_truth[:, idx] == 1).astype(int), prediction[:, idx])
...
def perstage_average_precision(ground_truth, prediction, class_names, ignore_index, metrics,
                                postprocessing):
    result = OrderedDict()
    ground_truth = np.array(ground_truth)
    prediction = np.array(prediction)
