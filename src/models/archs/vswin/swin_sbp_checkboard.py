...
from .sbp_ops_checkboard import (
    SBPMlpFunc,
    PFDotProductAttention_v4 as PFDotProductAttention,
)
...
def make_block_checkboard(D, H, W, block_size, downsample):
    ...
    return torch.from_numpy(tar_board).to(torch.bool)
...
class WindowAttention3D(nn.Module):
    ...
    def window_partition_grad_mask(self, grad_mask, orig_x_shape):
        ...
        Args:
            grad_mask (torch.Tensor[D,H,W]): the input grad_mask
            unpartitioned_x_shape (List): (B,D,H,W,C)

        Returns: torch.Tensor[num_windows, N]: the grad_mask for attention.  N = window_d x window_h x window_w
        ...
        D, H, W = grad_mask.shape
        grad_mask = grad_mask.reshape(
            D // window_d, window_d, H // window_h, window_h, W // window_w, window_w
        )
        grad_mask = grad_mask.permute(0, 2, 4, 1, 3, 5).reshape(
...
class SwinTransformerBlock3D(nn.Module):
    ...
    def forward(self, x, mask_matrix, grad_mask=None, downsample=None):
        ...
        Args:
            ...
            grad_mask (torch.Tensor[D,H,W]): mask for grad drop.
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, grad_mask, gd_downsample):
        ...
        Args:
            ...
            grad_mask: mask for grad, tensor size (1, D, H, W)
        """
...
class SwinTransformer3D(nn.Module):
    ...
    def generate_grad_mask(self, x, block_size, gd_downsample):
        ...
        Returns: torch.Tensor[D, H, W]. The mask for graddrop.

        """
        B, C, D, H, W = x.shape
        return make_block_checkboard(D, H, W, block_size, gd_downsample).to(x.device)
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.layers):
            # print(f'stage:{i}')
            block_size = 2 ** (len(self.layers) - i - 1)
            grad_mask = self.generate_grad_mask(x, block_size, self.gd_downsample)
